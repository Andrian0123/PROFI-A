...
- **Экран сканирования помещения** (`RoomScanScreen`) — пользователь подключает камеру, сканирует помещение или выбирает фото/файл; приложение отправляет данные на сервер и получает **автоматически определённые размеры**: высота стен, периметр, площадь пола.
...
- **Сканер документов** (`DocumentScanScreen`) — в меню «Документы → Сканер документов»: выбор фото документа → отправка на `POST /api/v1/scan/document` → отображение размеров (мм) и распознанного содержимого (см. docs/API_СКАНИРОВАНИЯ_ANDROID_PYTHON.md).
